...
nChannels: 16
Time Slot	A	B	C
0	if has(F0) push(F0: A -> B, #3)	wait(#3)	sleep
1	wait(#6)	if has(F0) push(F0: B -> C, #6) else pull(F0: A -> B, #6)	wait(#6)
2	sleep	if has(F0) push(F0: B -> C, #7)	wait(#7)
3	sleep	wait(#3)	if has(F1) push(F1: C -> B, #3)
4	wait(#8)	if has(F1) push(F1: B -> A, #8) else pull(F1: C -> B, #8)	wait(#8)
5	wait(#9)	if has(F1) push(F1: B -> A, #9)	sleep
6	sleep	sleep	sleep
7	sleep	sleep	sleep
8	sleep	sleep	sleep
9	sleep	sleep	sleep
